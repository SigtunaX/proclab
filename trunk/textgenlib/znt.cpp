...
#ifdef TEXTURE_EDITOR
//////////////////////////////////////////////////////////////////////
ZNT::ZNT(char file[], CTextGen &texture)
{
	t = &texture;
...
ZNT::~ZNT()
{
	if (data!=NULL)	// Potser no fa ni falta.... ens estalviariem 1 o 2 ks en memÚria, perÚ Ès mes codi! :D
		free(data);
}
...
void ZNT::LoadText(int numtex)
{
	...
	for (i=0; i<(t->dtex[numtex].font.p.numletters-1); i++)	// Posem el "-1" ja que el terminador no hi Ès
		t->dtex[numtex].font.text[i]	= data[data_size++];
	t->dtex[numtex].font.text[i] = '\0';	// Afegim el temrinador a m‡
